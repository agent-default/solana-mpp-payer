...
import { init, loadPrincipalSigner, loadSigner } from "./lib.js";

test("file signer loader binds the generated key to the principal with safe modes", async () => {
  ...
  try {
    ...
    const loaded = await loadPrincipalSigner(dir);

    assert.equal(signer.pubkey, principal.pubkey);
    assert.equal(signer.signer.address, principal.pubkey);
    assert.equal(signer.isDemo, false);
    assert.equal(loaded.signer.pubkey, signer.pubkey);
    ...
    await assert.rejects(loadSigner(dir), /must be mode 0600; refuse signer/);
  } finally {
